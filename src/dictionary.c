...
char *generate_dictionary(const node_t *huffman_tree) {
  ...
  while (idx > 0) {
    ...
    dictionary = temp;
    snprintf(dictionary + strlen(dictionary), 2, "%s", node->value ? "1" : "0");
    ...
    if (node->value) {
      ...
      temp = realloc(dictionary, strlen(dictionary) + strlen(binary_code) + 1);
      if (!temp) {
        free(binary_code);
        free(dictionary);
        return NULL;
      ...
      dictionary = temp;
      snprintf(dictionary + strlen(dictionary), strlen(binary_code) + 1, "%s",
               binary_code);
      free(binary_code);
    } else {
