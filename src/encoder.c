/**
 * @file encoder.c
 * @brief Implementation of the Huffman encoding and compression logic.
 *
 * This file contains the core functions for compressing a file using Huffman
 * coding. It takes a frequency-based Huffman tree, generates codes for each
 * character, writes the tree's structure (dictionary) and the encoded data
 * to an output file.
 */
...
void compress(char* dictionary, const char* codes[256], FILE* input_file,
    ...
    *xor_check = (unsigned char)'a';  // 'magic number' used later to check
    ...
    unsigned char input_buffer[BUFFER_LIMIT * 9], output_buffer[BUFFER_LIMIT],
    ...
    // Read original file
    while ((read = fread(bytes_from_input_file, 1, BUFFER_LIMIT, input_file)) >
           0) {
        for (size_t i = 0; i < read; i++) {
            ...
            if (input_buffer_length + code_length >= BUFFER_LIMIT * 9) {
                fprintf(stderr, "Input buffer overflow: %zu + %zu >= %d\n",
                        input_buffer_length, code_length, BUFFER_LIMIT * 9);
                exit(EXIT_FAILURE);
            }
        ...
        memmove(input_buffer, input_buffer + iteration_counter * BYTE_SIZE,
                input_buffer_length);
    }
...
char* save_dictionary(char* dictionary, FILE* output_file,
    ...
    char* rest_of_the_dictionary =
        write_first_two_bytes(dictionary, output_file);
    while (strlen(rest_of_the_dictionary) >= BYTE_SIZE) {
        get_eight_bits(rest_of_the_dictionary, eight_bits);
        output_buffer[output_buffer_length] =
            (unsigned char)strtol(eight_bits, NULL, 2);

        *xor_check ^= output_buffer[output_buffer_length];
        output_buffer_length++;

        memmove(rest_of_the_dictionary, rest_of_the_dictionary + BYTE_SIZE,
...
char* write_first_two_bytes(char* dictionary, FILE* output_file) {
    ...
    if (strlen(dictionary) >= BYTE_SIZE * 2) {
        ...
        for (size_t i = 0; i < 2; i++) {
            ...
            output_buffer[output_buffer_length++] =
                (unsigned char)strtol(eight_bits, NULL, 2);
            memmove(dictionary, dictionary + BYTE_SIZE,
...
void write_n_of_missing_bits(size_t n_of_missing_bits, FILE* output_file) {
    ...
    char* binary_string_of_n_of_missing_bits =
        char_binary_code((unsigned char)n_of_missing_bits, 3);
