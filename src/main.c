/**
 * @file main.c
 * @brief Main entry point for the Huffman compression and decompression
 * utility.
 *
 * This file handles command-line argument parsing and orchestrates the
 * compression or decompression process by calling the appropriate functions
 * from other modules.
 */
...
int main(int argc, char** argv) {
    ...
    if (strcmp(mode_str, "1") == 0) {  // decompression
        ...
        if (dictionary == NULL) {
            fprintf(stderr,
                    "Error: Failed to read dictionary from file '%s'.\n",
                    original_filename);
            fclose(original_file);
            fclose(output_file);
            return EXIT_FAILURE;
        ...
        if (huffman_tree == NULL) {
            fprintf(stderr,
                    "Error: Failed to build Huffman tree from dictionary.\n");
