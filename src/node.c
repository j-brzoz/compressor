/**
 * @file node.c
 * @brief Implementation of Huffman tree node operations.
 *
 * This file contains functions for creating and managing the nodes of a Huffman
 * tree. It includes functionality for node creation, freeing nodes, building
 * the tree from a heap, generating Huffman codes, and reconstructing the tree
...
node_t* make_huffman_tree_from_dict(dict_t* dictionary) {
    ...
    if (current_bit == '0') {
        ...
        strncpy(value_str, dictionary->dictionary + dictionary->index_helper,
                8);
        value_str[8] = '\0';
