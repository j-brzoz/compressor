...
int analyze_buffer(const unsigned char *buffer, const node_t *root,
  ...
  for (size_t code_length = 1; code_length <= buffer_length - buffer_start;) {
    ...
    // Find the key
    const unsigned char *key = get_key(root, single_code_buffer, code_length);
...
void decompress(FILE *input_file, FILE *output_file, const dict_t *dictionary,
                const node_t *root) {
  if (!input_file || !output_file || !dictionary || !root)
    return;
  ...
  // Read bytes
  while ((read_bytes =
              fread(bytes_from_input_file, 1, BUFFER_LIMIT, input_file)) > 0) {
    for (size_t i = 0; i < read_bytes; i++) {
      ...
      memcpy(binary_string_buffer + binary_string_buffer_length, eight_bits,
             BYTE_SIZE);
